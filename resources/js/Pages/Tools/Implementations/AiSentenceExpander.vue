    ...
    <AiToolWrapper :remaining="remainingRequests">
        <div class="bg-white dark:bg-surface-800 p-6 rounded-2xl shadow-sm border border-surface-200 dark:border-surface-700">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-2">Short Sentence or Thought</label>
                        <div class="relative">
                            <textarea 
                                v-model="inputText"
                                rows="3"
                                ...
                                placeholder="E.g. The sun set over the mountains."
                            ></textarea>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-medium text-surface-500 mb-1">Style / Tone</label>
                        ...
                        <div>
                            <label class="block text-xs font-medium text-surface-500 mb-1">Expansion Level</label>
                            <select v-model="options.length" class="block w-full rounded-lg border-surface-300 dark:border-surface-600 bg-white dark:bg-surface-800 text-sm focus:ring-primary-500 focus:border-primary-500">
                                <option value="Add details (1 paragraph)">Add details (1 paragraph)</option>
                                <option value="Highly detailed (2 paragraphs)">Highly detailed (2 paragraphs)</option>
                            </select>
                        </div>
                    </div>
                    ...
                    >
                        ...
                        <svg v-else class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4" /></svg>
                        {{ isLoading ? 'Expanding...' : 'Expand Sentence' }}
                    </button>
                ...
                <div class="space-y-4">
                    <div class="flex justify-between items-center">
                        <label class="block text-sm font-medium text-surface-700 dark:text-surface-300">Expanded Text</label>
                    ...
                    <div class="relative h-full min-h-[300px]">
                        ...
                        <div v-else-if="isLoading" class="absolute inset-0 border border-surface-200 dark:border-surface-700 rounded-xl flex flex-col items-center justify-center bg-surface-50 dark:bg-surface-900/50">
                            ...
                            <p class="mt-4 text-surface-500 font-medium animate-pulse">Adding details...</p>
                        </div>
...
const options = ref({
    ...
    length: 'Add details (1 paragraph)'
});
