<template>
    <div class="bg-white dark:bg-surface-800 p-6 md:p-8 rounded-2xl shadow-sm border border-surface-200 dark:border-surface-700">
        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            <!-- Output Area -->
            <div>
                <label class="block text-sm font-bold text-surface-900 dark:text-white mb-2">MD5 Hash Result</label>
                <div class="flex items-center gap-2">
                    <div class="flex-1 p-4 rounded-xl border border-surface-200 dark:border-surface-700 bg-surface-100/50 dark:bg-surface-800/50 overflow-x-auto">
                        <p class="font-mono text-sm break-all text-surface-900 dark:text-white select-all">
                            {{ hashResult || 'd41d8cd98f00b204e9800998ecf8427e' }}
                        </p>
                    </div>
        ...
        <div class="mt-8 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-xl border border-yellow-100 dark:border-yellow-800/30 text-sm text-yellow-800 dark:text-yellow-300">
            <p><strong>Note:</strong> MD5 is no longer considered secure for password hashing or cryptographic signatures. It is best used for checksums and non-cryptographic validations.</p>
        </div>
    </div>
</template>
...
// Lightweight JS MD5 implementation
const md5 = function() {
    ...
    function hex(x) {
        for (var i = 0; i < x.length; i++) x[i] = ((x[i] >> 0) & 0xFF).toString(16).padStart(2, '0') + ((x[i] >> 8) & 0xFF).toString(16).padStart(2, '0') + ((x[i] >> 16) & 0xFF).toString(16).padStart(2, '0') + ((x[i] >> 24) & 0xFF).toString(16).padStart(2, '0');
        return x.join('');
    }
}();
...
watch(inputText, (newVal) => {
    ...
    hashResult.value = md5(newVal);
}, { immediate: true });

const copyToClipboard = async () => {
    const textToCopy = hashResult.value || 'd41d8cd98f00b204e9800998ecf8427e';
    try {
