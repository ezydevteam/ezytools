    ...
    <AiToolWrapper :remaining="remainingRequests">
        <div class="bg-white dark:bg-surface-800 p-6 rounded-2xl shadow-sm border border-surface-200 dark:border-surface-700">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-2">Job Description / Requirements</label>
                        <div class="relative">
                            <textarea 
                                v-model="inputText"
                                rows="5"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-medium text-surface-500 mb-1">Job Title</label>
                            <input v-model="options.job_title" type="text" class="block w-full rounded-lg border-surface-300 dark:border-surface-600 bg-white dark:bg-surface-800 text-sm focus:ring-primary-500 focus:border-primary-500" placeholder="Software Engineer">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-surface-500 mb-1">Company Name</label>
                            <input v-model="options.company" type="text" class="block w-full rounded-lg border-surface-300 dark:border-surface-600 bg-white dark:bg-surface-800 text-sm focus:ring-primary-500 focus:border-primary-500" placeholder="Google">
                        </div>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-2">Your Experience & Skills</label>
                        <div class="relative">
                            <textarea 
                                v-model="options.experience"
                                rows="3"
                                ...
                                placeholder="Briefly list your key skills and past roles..."
                            ></textarea>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-medium text-surface-500 mb-1">Your Name</label>
                            <input v-model="options.name" type="text" class="block w-full rounded-lg border-surface-300 dark:border-surface-600 bg-white dark:bg-surface-800 text-sm focus:ring-primary-500 focus:border-primary-500" placeholder="John Doe">
                        </div>
                        <div>
                    ...
                    <button 
                        ...
                        :disabled="!inputText || !options.job_title || isLoading"
                    ...
                    >
                        ...
                        {{ isLoading ? 'Writing Letter...' : 'Write Cover Letter' }}
                    </button>
                ...
                <div class="space-y-4">
                    <div class="flex justify-between items-center">
                        <label class="block text-sm font-medium text-surface-700 dark:text-surface-300">Generated Cover Letter</label>
                    ...
                    <div class="relative h-full min-h-[500px]">
                        ...
                        <div v-else-if="isLoading" class="absolute inset-0 border border-surface-200 dark:border-surface-700 rounded-xl flex flex-col items-center justify-center bg-surface-50 dark:bg-surface-900/50">
                            ...
                            <p class="mt-4 text-surface-500 font-medium animate-pulse">Drafting letter...</p>
                        </div>
...
const handleGenerate = () => {
    const message = `
Job Title: ${options.value.job_title}
Company: ${options.value.company}
Applicant Name: ${options.value.name}
Applicant Experience: ${options.value.experience}
Job Description:
${inputText.value}
    `;
    generate(props.tool.slug, message, { tone: options.value.tone });
};
