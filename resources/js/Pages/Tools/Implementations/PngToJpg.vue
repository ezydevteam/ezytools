<template>
    <div class="bg-white dark:bg-surface-800 p-6 md:p-8 rounded-2xl shadow-sm border border-surface-200 dark:border-surface-700">
        ...
        <div v-if="!selectedFile" class="border-2 border-dashed border-surface-300 dark:border-surface-600 rounded-2xl p-12 text-center hover:bg-surface-50 dark:hover:bg-surface-900/50 transition-colors cursor-pointer relative" @dragover.prevent="dragover = true" @dragleave.prevent="dragover = false" @drop.prevent="handleDrop" :class="{ 'bg-primary-50 dark:bg-primary-900/20 border-primary-400': dragover }">
            ...
            <div class="w-20 h-20 bg-gradient-to-br from-sky-100 to-blue-100 text-sky-600 rounded-full flex items-center justify-center mx-auto mb-4 dark:from-sky-900/50 dark:to-blue-900/50 dark:text-sky-400">
            ...
            <p class="text-sm text-surface-500 dark:text-surface-400">Click or drag a PNG file to convert to JPG</p>
        </div>
        ...
        <div v-else class="space-y-8">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-surface-500">{{ formatBytes(selectedFile.size) }} • PNG</p>
                    </div>
                </div>
            ...
            <div class="flex items-center justify-center gap-4 p-6 bg-surface-50 dark:bg-surface-900 rounded-xl border border-surface-200 dark:border-surface-700">
                ...
                <svg class="w-8 h-8 text-sky-500 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>
                <div class="flex flex-col items-center gap-2">
                ...
                <div class="space-y-6">
                    <div>
                        <div class="flex justify-between mb-2">
                            ...
                            <span class="text-sm font-bold text-sky-600 dark:text-sky-400">{{ quality }}%</span>
                        </div>
                        <input type="range" v-model="quality" min="10" max="100" class="w-full h-2 bg-surface-200 rounded-lg appearance-none cursor-pointer dark:bg-surface-700 accent-sky-600" @change="convertImage">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-3">Background Color</label>
                        <p class="text-xs text-surface-500 mb-2">PNG transparency will be replaced with this color</p>
                        <div class="flex items-center gap-3">
                            <input type="color" v-model="bgColor" @input="convertImage" class="w-10 h-10 rounded cursor-pointer border-0 p-0">
                            <span class="text-sm font-mono text-surface-600 dark:text-surface-400">{{ bgColor }}</span>
                        </div>
                    </div>
                    ...
                    <div v-if="convertedSize && selectedFile" class="p-4 bg-surface-50 dark:bg-surface-900 rounded-xl border border-surface-200 dark:border-surface-700">
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-surface-600 dark:text-surface-400">Size Reduction:</span>
                            <span class="font-black text-lg" :class="savedPercentage > 0 ? 'text-green-600 dark:text-green-400' : 'text-surface-500'">{{ savedPercentage }}%</span>
                        </div>
                    </div>
