    ...
    <div class="bg-white dark:bg-surface-800 p-6 md:p-8 rounded-2xl shadow-sm border border-surface-200 dark:border-surface-700">
        <div class="mb-6">
            <div class="flex p-1 bg-surface-100 dark:bg-surface-900 rounded-xl max-w-md mx-auto">
                <button @click="mode = 'encode'" :class="[mode === 'encode' ? 'bg-white dark:bg-surface-700 text-primary-600 dark:text-primary-400 shadow-sm' : 'text-surface-600 dark:text-surface-400 hover:text-surface-900 dark:hover:text-white']" class="flex-1 py-2 text-sm font-medium rounded-lg transition-all">Encode URL</button>
                <button @click="mode = 'decode'" :class="[mode === 'decode' ? 'bg-white dark:bg-surface-700 text-primary-600 dark:text-primary-400 shadow-sm' : 'text-surface-600 dark:text-surface-400 hover:text-surface-900 dark:hover:text-white']" class="flex-1 py-2 text-sm font-medium rounded-lg transition-all">Decode URL</button>
            </div>
        </div>
...
const outputText = computed(() => {
    ...
    try {
        if (mode.value === 'encode') {
            ...
            return decodeURIComponent(inputText.value.replace(/\+/g, ' '));
        }
    } catch (e) {
        error.value = 'Invalid URI Component. Unable to decode.';
        return '';
    }
});
