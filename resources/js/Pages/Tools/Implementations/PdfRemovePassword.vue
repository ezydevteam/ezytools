<template>
    <div class="max-w-4xl mx-auto py-8">
        <h1 class="text-3xl font-bold text-surface-900 dark:text-white mb-2">Unlock PDF</h1>
        <p class="text-surface-600 dark:text-surface-400 mb-8">Remove password security from your PDF file so it can be opened freely.</p>
        ...
        <div v-else class="bg-white dark:bg-surface-800 rounded-2xl shadow-sm border border-surface-200 dark:border-surface-700 overflow-hidden">
            ...
            <div class="p-6 md:p-8 space-y-6">
                <div>
                    ...
                    <p class="text-xs text-surface-500 mt-2">We need the correct password to unlock and strip the security from this file.</p>
                </div>
                ...
                <div class="flex justify-end pt-4 border-t border-surface-100 dark:border-surface-700">
                    <button 
                        ...
                        class="bg-gradient-to-r from-red-500 to-orange-500 hover:from-red-600 hover:to-orange-600 text-white shadow-lg hover:shadow-xl shadow-red-500/30 transition-all duration-300 px-8 py-3 rounded-xl text-lg font-medium flex items-center gap-2"
                        :class="{'opacity-50 cursor-not-allowed': isProcessing || !password}"
                    >
        ...
        <div v-if="downloadUrl" class="fixed inset-0 z-50 flex items-center justify-center bg-surface-900/80 backdrop-blur-sm p-4">
            <div class="bg-white dark:bg-surface-800 rounded-3xl shadow-2xl p-8 md:p-12 max-w-md w-full text-center relative overflow-hidden">
                ...
                <h3 class="text-2xl font-bold text-surface-900 dark:text-white mb-2">Unlocked!</h3>
                <p class="text-surface-600 dark:text-surface-400 mb-8">Your PDF password has been removed successfully.</p>
...
import { LockOpenIcon, ArrowPathIcon, CheckCircleIcon, ArrowDownTrayIcon, EyeIcon, EyeSlashIcon } from '@heroicons/vue/24/outline';
...
const unlockPdf = async () => {
    ...
    try {
        const response = await axios.post('/api/pdf/remove-password', formData, {
        ...
        if (response.data.success) {
            ...
            processError.value = response.data.message || 'An error occurred while unlocking the PDF.';
        }
    } catch (err) {
