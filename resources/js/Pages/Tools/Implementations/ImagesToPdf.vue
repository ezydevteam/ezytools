<template>
    <div class="max-w-4xl mx-auto py-8">
        <h1 class="text-3xl font-bold text-surface-900 dark:text-white mb-2">Images to PDF</h1>
        <p class="text-surface-600 dark:text-surface-400 mb-8">Convert JPG, PNG, and other images into a single PDF document.</p>
        ...
        <div v-if="files.length === 0" class="bg-white dark:bg-surface-800 rounded-2xl shadow-sm border border-surface-200 dark:border-surface-700 p-8">
            <PdfUploadZone 
                ...
                @error="error = $event"
                :max-files="tool.settings?.free_max_files || 20"
        ...
        <div v-else class="bg-white dark:bg-surface-800 rounded-2xl shadow-sm border border-surface-200 dark:border-surface-700 overflow-hidden">
            <div class="p-6 md:p-8 border-b border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-900/50 flex flex-col md:flex-row items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h3 class="font-semibold text-surface-900 dark:text-white">{{ files.length }} Images Selected</h3>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <input type="file" ref="addMoreInput" class="hidden" accept="image/*" multiple @change="addMoreFiles" />
            ...
            <div class="p-6 md:p-8 space-y-6">
                ...
                >
                    <template #item="{ element, index }">
                        <div class="relative group aspect-square rounded-xl overflow-hidden border-2 border-surface-200 dark:border-surface-700 hover:border-primary-500 cursor-move transition-colors">
                            <img :src="element.preview" class="w-full h-full object-cover" />
                            <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                <button @click="removeFile(index)" class="p-2 bg-red-500 text-white rounded-full hover:bg-red-600 transition-colors shadow-lg">
                                    <XMarkIcon class="w-5 h-5" />
                                </button>
                            </div>
                            <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-2 text-white text-xs truncate">
                                {{ element.file.name }}
                            </div>
                            <div class="absolute top-2 left-2 w-6 h-6 bg-primary-500 text-white rounded-full flex items-center justify-center text-xs font-bold shadow">
                                {{ index + 1 }}
                            </div>
                        </div>
                    </template>
        ...
        <div v-if="downloadUrl" class="fixed inset-0 z-50 flex items-center justify-center bg-surface-900/80 backdrop-blur-sm p-4">
            <div class="bg-white dark:bg-surface-800 rounded-3xl shadow-2xl p-8 md:p-12 max-w-md w-full text-center relative overflow-hidden">
                ...
                <p class="text-surface-600 dark:text-surface-400 mb-8">Your images have been converted to a single PDF.</p>
...
import { PhotoIcon, ArrowPathIcon, CheckCircleIcon, ArrowDownTrayIcon, XMarkIcon, DocumentTextIcon } from '@heroicons/vue/24/outline';
...
const onFilesSelected = (newFiles) => {
    ...
    const newItems = Array.from(newFiles).map(f => ({
        file: f,
        name: f.name,
