    ...
    <AiToolWrapper :remaining="remainingRequests">
        <div class="bg-white dark:bg-surface-800 p-6 rounded-2xl shadow-sm border border-surface-200 dark:border-surface-700">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-2">Page Title or Topic</label>
                        <div class="relative">
                            <input 
                                v-model="inputText"
                                type="text"
                                class="block w-full rounded-xl border-surface-300 dark:border-surface-600 bg-surface-50 dark:bg-surface-900 text-surface-900 dark:text-white focus:ring-primary-500 focus:border-primary-500 transition-colors text-base p-4"
                    ...
                    <div>
                        <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-2">Primary Keyword (Optional)</label>
                        <input v-model="options.keyword" type="text" class="block w-full rounded-lg border-surface-300 dark:border-surface-600 bg-surface-50 dark:bg-surface-900 text-surface-900 dark:text-white focus:ring-primary-500 focus:border-primary-500 text-sm" placeholder="running shoes">
                    </div>
                    ...
                    >
                        ...
                        <svg v-else class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" /></svg>
                ...
                <div class="space-y-4">
                    <div class="flex justify-between items-center">
                        ...
                        <div v-if="result" class="flex items-center gap-2">
                            <span :class="{'text-red-500 font-bold': result.length > 160, 'text-green-500': result.length <= 160 && result.length > 120}" class="text-xs text-surface-500">{{ result.length }}/160 chars</span>
                    ...
                    <div class="relative h-full min-h-[250px]">
                        ...
                        <div v-else class="h-full flex flex-col gap-4">
                            <textarea 
                                v-model="result"
                                readonly
                                class="block w-full h-32 rounded-xl border-surface-300 dark:border-surface-600 bg-white dark:bg-surface-800 text-surface-900 dark:text-white focus:ring-primary-500 focus:border-primary-500 transition-colors resize-none text-base p-4 whitespace-pre-wrap"
                            ></textarea>
                            
                            <!-- Google Preview -->
                            <div class="border border-surface-200 dark:border-surface-700 rounded-lg p-4 bg-white dark:bg-surface-900 mt-auto">
                                <p class="text-xs text-surface-500 mb-2 font-medium">Google Search Preview</p>
                                <div class="text-[14px] text-surface-900 dark:text-surface-300">
                                    <div class="text-[12px] text-surface-600 dark:text-surface-400 flex items-center gap-2 mb-1">
                                        <div class="w-4 h-4 bg-surface-200 dark:bg-surface-700 rounded-full"></div>
                                        <span>https://example.com › {{ inputText.toLowerCase().replace(/[^a-z0-9]+/g, '-') }}</span>
                                    </div>
                                    <div class="text-[20px] text-[#1a0dab] dark:text-[#8ab4f8] cursor-pointer hover:underline truncate">{{ inputText || 'Your Page Title Here' }}</div>
                                    <div class="text-[14px] text-[#4d5156] dark:text-[#bdc1c6] mt-1 line-clamp-2">{{ result }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
