<template>
    <div class="max-w-4xl mx-auto py-8">
        <h1 class="text-3xl font-bold text-surface-900 dark:text-white mb-2">PDF to Word</h1>
        <p class="text-surface-600 dark:text-surface-400 mb-8">Convert your PDF files to editable Word documents (DOCX).</p>
        ...
        <div v-else class="bg-white dark:bg-surface-800 rounded-2xl shadow-sm border border-surface-200 dark:border-surface-700 overflow-hidden">
            ...
            <div class="p-6 md:p-8 space-y-6">
                <div class="p-4 bg-surface-100 dark:bg-surface-900 rounded-xl text-surface-600 dark:text-surface-400 text-sm">
                    <strong>Note:</strong> This tool extracts the text from your PDF and converts it into a Word document. Complex layouts, tables, and images may not be perfectly preserved.
                </div>
        ...
        <div v-if="downloadUrl" class="fixed inset-0 z-50 flex items-center justify-center bg-surface-900/80 backdrop-blur-sm p-4">
            <div class="bg-white dark:bg-surface-800 rounded-3xl shadow-2xl p-8 md:p-12 max-w-md w-full text-center relative overflow-hidden">
                ...
                <p class="text-surface-600 dark:text-surface-400 mb-8">Your PDF has been successfully converted to a Word document.</p>
...
import { ArrowPathIcon, CheckCircleIcon, ArrowDownTrayIcon, DocumentTextIcon } from '@heroicons/vue/24/outline';
...
const convertToWord = async () => {
    ...
    try {
        const response = await axios.post('/api/pdf/pdf-to-word', formData, {
