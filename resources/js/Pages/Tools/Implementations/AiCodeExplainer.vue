                ...
                <div class="space-y-4">
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            <label class="block text-sm font-medium text-surface-700 dark:text-surface-300">Code Snippet</label>
                            <span class="text-xs text-surface-500 font-mono">{{ inputText.length }} chars</span>
                        </div>
                        <div class="relative">
                            <textarea 
                                v-model="inputText"
                                rows="12"
                                class="block w-full rounded-xl border-surface-300 dark:border-surface-600 bg-surface-900 text-surface-200 focus:ring-primary-500 focus:border-primary-500 transition-colors resize-y text-sm p-4 font-mono leading-relaxed"
...
function calculateTotal(price, taxRate) {
    return price + (price * taxRate);
                            ...
                            <button v-if="inputText" @click="inputText = ''" class="absolute top-3 right-3 text-surface-400 hover:text-white p-1 bg-surface-800 rounded-lg transition-colors" title="Clear code">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select v-model="options.audience" class="block w-full rounded-lg border-surface-300 dark:border-surface-600 bg-white dark:bg-surface-800 text-sm focus:ring-primary-500 focus:border-primary-500">
                                <option value="Beginner">Beginner (Simple words)</option>
                                <option value="Intermediate">Intermediate Developer</option>
                                <option value="Expert">Expert (Technical deep dive)</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-surface-500 mb-1">Focus on</label>
                            <select v-model="options.focus" class="block w-full rounded-lg border-surface-300 dark:border-surface-600 bg-white dark:bg-surface-800 text-sm focus:ring-primary-500 focus:border-primary-500">
                                <option value="General Explanation">General Explanation</option>
                                <option value="Finding Bugs">Finding Bugs / Security</option>
                    ...
                    >
                        ...
                        <svg v-else class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" /></svg>
                        {{ isLoading ? 'Analyzing Code...' : 'Explain Code' }}
                    </button>
                ...
                <div class="space-y-4">
                    <div class="flex justify-between items-center">
                        <label class="block text-sm font-medium text-surface-700 dark:text-surface-300">Explanation</label>
                    ...
                    <div class="relative h-full min-h-[400px]">
                        ...
                        <div v-else-if="isLoading" class="absolute inset-0 border border-surface-200 dark:border-surface-700 rounded-xl flex flex-col items-center justify-center bg-surface-50 dark:bg-surface-900/50">
                            ...
                            <p class="mt-4 text-surface-500 font-medium animate-pulse">Analyzing logic...</p>
                        </div>
                        ...
                        <textarea 
                            ...
                            v-model="result"
                            readonly
                            class="block w-full h-full rounded-xl border-surface-300 dark:border-surface-600 bg-white dark:bg-surface-800 text-surface-900 dark:text-white focus:ring-primary-500 focus:border-primary-500 transition-colors resize-none text-base p-4 whitespace-pre-wrap font-sans"
                        ></textarea>
                    </div>
                </div>
            </div>
...
const handleGenerate = () => {
    // Add specific instruction context
    const fullMessage = `Code:\n\`\`\`\n${inputText.value}\n\`\`\`\n\nAudience: ${options.value.audience}\nFocus: ${options.value.focus}`;
    generate(props.tool.slug, fullMessage, options.value);
};
